...
from .art_dtrack.art_dtrack_receiver import ArtDtrackReceiver, DtrackBody, DtrackMeasurementTool, DtrackMeasurementToolReference
...
class ArtDtrackTfPublisher(Node):
    ...
    def __init__(self):
        ...
        self.bodies = self.declare_parameter(
            'bodies', ['Tree Target 2','pen', 'antlers']).get_parameter_value().string_array_value
        self.measurement_tools = self.declare_parameter(
            'measurement_tools', ['M1','M2']).get_parameter_value().string_array_value
        self.measurement_tool_reference_bodies = self.declare_parameter(
            'measurement_tool_reference_bodies', ['MR1','MR2']).get_parameter_value().string_array_value
        ...
        while rclpy.ok():
            try:
                ...
                for body in msg.measurement_tool_reference_bodies:
                    if body.id >= 0 and body.id < len(self.measurement_tool_reference_bodies):
    ...
    def mt2tf(self, body: DtrackMeasurementTool, ts: float):
        t = TransformStamped()
        # t.header.frame_id = self.frame
        t.header.frame_id = self.measurement_tool_reference_bodies[body.id] # use MT reference body with same ID, make sure to align M1 with MR1 etc. 
