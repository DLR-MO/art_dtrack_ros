...
class DtrackMessage:
    ...
    measurement_tool_reference_bodies: list[DtrackMeasurementToolReference] = field(default_factory=list)
...
def parseDtrackLine(line: str) -> DtrackMessage:
    ...
    for row in line.splitlines():
        ...
        if data[0] == "fr":
            ...
            for i in range(num_tracked):
                block_id, block_position, block_3x3, block_button, block_covariance = blocks[i*5:i*5+5]
                ...
                body.covariance = np.array([[arr[0],arr[1],arr[2]],[arr[1],arr[3],arr[4]],[arr[2],arr[3],arr[5]]]) 
